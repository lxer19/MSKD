...
class GAT(nn.Module):
    def __init__(self, g, num_layers, in_dim, num_hidden, num_classes, heads, activation, feat_drop, attn_drop, negative_slope, residual,device):
    ...
    def forward(self, inputs, middle=False):
        ...
        for l in range(self.num_layers):
            h = self.gat_layers[l](self.g.to(torch.device(self.device)), h).flatten(1)
            middle_feats.append(h)
            h = self.activation(h)
        logits = self.gat_layers[-1](self.g.to(torch.device(self.device)), h).mean(1)
        if middle:
...
class GCN(nn.Module):
    def __init__(self, g, num_layers, in_dim, num_hidden, num_classes,device):
    ...
    def forward(self, inputs, middle=False):
        ...
        middle_feats = []
        for l in range(self.num_layers):
            h = self.gat_layers[l](self.g.to(torch.device(self.device)), h)
            middle_feats.append(h)
            h = F.relu(h)
        logits = self.gat_layers[-1](self.g.to(torch.device(self.device)), h)
        if middle:
            return logits, middle_feats
        return logits